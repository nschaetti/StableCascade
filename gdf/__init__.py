...
class GDF():
    ...
    def sample(
        ...
        logSNR_range = schedule(r_range, shift=shift)[:, None].expand(
            -1,
            shape[0] if x_init is None else x_init.size(0)
        ).to(device)
        ...
        # CFG given
        if cfg is not None:
            # Create empty unconditional inputs
        ...
        for i in range(0, timesteps):
            ...
            if cfg is not None and \
                    (cfg_t_stop is None or r_range[i].item() >= cfg_t_stop) and \
                    (cfg_t_start is None or r_range[i].item() <= cfg_t_start):
                # CFG value
                cfg_val = cfg

                # CFG is a list or tuple
                ...
                # torch.lerp: does a linear interpolation of two tensors
                # pred_cfg = pred_unconditional + cfg_val * (pred - pred_unconditional)
